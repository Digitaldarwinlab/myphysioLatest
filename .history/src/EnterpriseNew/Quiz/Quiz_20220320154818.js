...
function Quiz() {
  ...
  const clickHandler = (e) => {
    setSelected(true);
    const q = newQue[question].exercise_question;
    const qId = newQue[question].id;
    const ans = e.target.value;
    const section = newQue[question].exercise_name;
    const qAns = {
      questionId: qId,
      question: q,
      answer: ans,
  ...
  const classHandler = (index) => {
    if (questionAnswers.length > 0) {
      if (questionAnswers[question]) {
        if (
          newQue[question].options[index] ===
          newQue[question].options[questionAnswers[question].answer]
        ) {
          console.log("active");
  ...
  const backHandler = () => {
    dispatch({ type: "CLEAR" });
    history.push("/patient/enterprize/body-selection");
  };
    ...
    <div className="App">
      ...
      <section id="question_section">
        <div className="questions">
          ...
          {question < newQue.length ? (
            <div className="question">
              <div className="question_number">
                <div className="dots">
                  {newQue.map((question, index) => (
              ...
              <p>
                {newQue[question].exercise_question
                  .toLowerCase()
                  ...
                  ? newQue[question].exercise_question +
                    ` ${newQue[question].exercise_name}?`
                  : newQue[question].exercise_question}
              </p>
              <div className="options">
                {newQue[question].options.map((option, index) => (
                  <button
                    className={
            ...
            <button className="" onClick={() =>history.push('/PoseTest')}>Next</button></div>
          ) : (
            <>
              {!isLoading ? (
                <div className="end">
