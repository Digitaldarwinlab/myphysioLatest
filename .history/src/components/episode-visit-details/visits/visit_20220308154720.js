...
const columns = [
    ...
    },
    {
        ...
        align: "center",
        resposive: ["lg", "md", "sm", "xs"],
        render: link => <a href={"/physio"+link} target="_blank" rel="noopener noreferrer">{link}</a>
    }
    ,{
        title: "Delete",
        dataIndex: "Delete",
    ...
    if (state.patient_code) {
        ...
        const filteredData1 = newdata.map((val, index) => {
            ...
            return {
                ...
                edit: <BiEdit className="iconClass3" title="Edit" onClick={() => handleEdit(val.pp_vd_id)} />,
                conLink: val.video_link,
                Delete:<Button className=""  type="primary"  danger  onClick={()=>showDeleteConfirm(val)}><i class="fa fa-trash" aria-hidden="true"></i></Button>
            }
        });
