...
client.on("stream-subscribed", function (evt) {
  ...
  if ($("#full-screen-video").is(":empty")) {
    ...
    try {
      ...
      console.log(
        "Subscribe remote stream successfully else : " + width,
        " ",
        screenShareActive
      );
      $("#player_" + remoteId).css({
        position: "unset",
        "background-color": "",
      });
      $("#video" + remoteId).css({
      ...
      console.log(
        "Subscribe remote stream successfully else : " + remoteStream.getId()
      );
    } catch (err) {
...
client.on("peer-leave", function (evt) {
  ...
  if (remoteStreams[streamId] != undefined) {
    ...
    if (streamId == mainStreamId) {
      ...
      setTimeout(() => {
        remoteStreams[streamId].play("full-screen-video"); // play the random stream as the main stream
        mainStreamId = streamId; // set the new main remote stream
      }, 3000);
    } else {
...
function initScreenShare(agoraAppId, channelName, uid) {
  ...
  screenClient.on("stream-published", function (evt) {
    ...
    if ($("#full-screen-video").is(":empty")) {
      ...
      // move the current main stream to miniview
      console.log(
        "Publish screen stream successfully else",
