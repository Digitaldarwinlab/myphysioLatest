...
const Register1 = (props) => {
  ...
  useEffect(() => {
    ...
    if (data.DOB) {
      ...
      form.setFieldsValue({
        Age: calculate(new Date(), new Date(moment(data.DOB, "YYYY-MM-DD"))),
      });
      dispatch({
        type: STATECHANGE,
        payload: {
          ...
          value: calculate(
            new Date(),
            new Date(moment(data.DOB, "YYYY-MM-DD"))
          ),
        },
      });
    }
  ...
  const handleBlur = (e) => {
    ...
    if (key === "FirstName" || key === "MiddleName" || key === "LastName") {
      ...
      if (error.error) {
        if (key === "FirstName") error.error = "First " + error.error;
        else if (key == "MiddleName" && e.target.value.length > 0) {
          error.error = "Middle " + error.error;
        } else error.error = "Last " + error.error;
      }
  ...
  const onFinish = () => {
    ...
    if (validation.checkNameValidation(data.FirstName).error) {
      dispatch({
        type: VALIDATION,
        payload: {
          error: "First" + validation.checkNameValidation(data.FirstName).error,
        },
      });
      ...
      dispatch({
        type: VALIDATION,
        payload: {
          error: "Last" + validation.checkNameValidation(data.LastName).error,
        },
      });
  ...
  return (
    <>
      ...
      <StepBar src={svg} />
      <Title level={4} className="border mb-0 p-2">
        Basic Information
      </Title>
