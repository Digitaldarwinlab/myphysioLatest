...
import { GetPatientCurrentEpisode,GetCalanderDataApi } from '../../PatientAPI/PatientDashboardApi';
...
//TimeColors
const timeColors = ["#17BD9C", "#00294C", "#17BD9C", "#00294C", "#17BD9C", "#00294C"];
//Selected Time Style
const selectedStyle = {
...
const PatCalendar = ({ onChangeVideoUrl }) => {
    ...
    const [customisedDate,SetcustomisedDate]=useState(new Date().toISOString().substring(0, 10))
  ...
  //  console.log('date is : ' + todaysdate.getDate())
    ...
    const combineTwoCarePlan = (data) => {
      ...
      //  console.log(data)        
            ...
            for (let i = 0; i < data[k].time_slot.length; i++) {
              ...
              //  console.log('inside if')                
                ...
                //  console.log(  checkTimeToMapExercise)
                    ...
                    commonTime[data[k].time_slot[i]] = [...commonTime[data[k].time_slot[i]], ...notPresentExercise];               
                     //    console.log('data[k]')
                  //  console.log(data[k].time_slot[i][0])
                } else {
                    console.log('loop 4 s')
                        console.log(data[k].pp_cp_id + ' : time_slot is : '+ data[k].time_slot[i])
                        let temparray=careplanIdArray
                        temparray.push({time:data[k].time_slot[i] , careplanId:data[k].pp_cp_id })
                 ...
                 //   console.log(data[k].time_slot[i])
                    ...
                    for (let j = 0; j < exercises.length; j++) {
                        ...
                        if (data[k].time_slot[i] in checkTimeToMapExercise) {
                            checkTimeToMapExercise[data[k].time_slot[i]][exercises[j].ex_em_id] = 1;
        ...
        times = times.sort((a, b) => a.charCodeAt(a.length - 2) - b.charCodeAt(b.length - 2));
    ...
    useEffect(()=>{
        ...
        if(allvisits.length>0)
        {
            const sortVisits=allvisits.filter((data)=>{
                ...
                const date=datetypof=='number' ? new Date(data.appointment_detail.startDate).toISOString().substring(0,10) :  data.appointment_detail.startDate.toString().substring(0,10)
              //  console.log( 'date is')
              //  console.log(date)
  ...
  //  console.log(currentEpissode)
   ...
   // console.log(exercises)
    ...
    const TimeSlots = (times) => {
        return (
            <div className="p-2  border   text-start exercise-card" id="exercise-card" >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-start" }}>
                    <h3 className="p fw-bold">{selectedDate ? day(new Date(selectedDate).getDay()) : day(new Date().getDay())}</h3>
                    <h3 className="p fw-bold ms-2">{selectedDate ? customisedDate : customisedDate}</h3>
                </div>
                <div className="time-slot-buttons" style={{ display: "flex", justifyContent: "space-around" }}>
                    {times.map((time, index) => {
                        return (
                            <Button key={index}
                                ...
                                } : selectedTime === index ? selectedStyle : UnselectedStyle}
                                onClick={() => {
    ...
    const handleClick = (exercise) => {
        exercise['ChoosenTime']= chosenTime ? chosenTime : Object.keys(mappedTimeToExercises)[0] ? Object.keys(mappedTimeToExercises)[0] : times[selectedTime]
        exercise['careplanId']=careplanIdArray[selectedTime]
        onChangeVideoUrl(exercise.video_url);
        history.push({
            pathname: '/patient/exercises/brief', state: {
                exercise
            }
        });
    }
    ...
    const ExerciseCard = (exercise) => {
        ...
        return (
            <div className="main-exercise-card" id="main-exercise-card" >
                <img src={`${process.env.REACT_APP_EXERCISE_URL}/${exercise.image_url}`} alt={`image_${exercise.ex_em_id}`}
                    width={120} height={120} style={{ borderRadius: 60 }}
                />
                <div className="ms-4 main-exercise-card" id="main-exercise-card">
                    ...
                    <Button
                        ...
                        style={{
                            ...btnStyle,
                            ...{
                                cursor: customisedDate!==convert(new Date()) ? "not-allowed" : "pointer",
                                backgroundColor: customisedDate!==convert(new Date()) ? "gray" : "#00022e"
                            }
                        }}
        ...
        return (
            <div className="p-2  border visit-card-2" id="visit-card-2" >
                    <div className="prescription-row">
                        <span className="presription-col"><b>Prescription Detail  : </b> 2 Capsules  of Peracetamol and Syrup  </span>
                    </div>
                    <div className="presription-row">
                    <div className="prescription-row">
                        <span className="presription-col"><b>Other Detail</b> Twice a Day after the meal </span>
                    </div>
                </div>
   ...
   var datetypof
    ...
    {   
            ...
            date=datetypof=='number' ? new Date(data.appointment_detail.startDate).toISOString().substring(0,10) :  data.appointment_detail.startDate.toString().substring(0,10)
    ...
    try{
        return (
            ...
            <p className="w-100 text-start" style={{textAlign:'left',fontSize:'24px',position:'relative',left:'10px'}}><b>Visit Details</b></p>
            <div className="upper-visit-row">
                <span className="visit-col" span={10} > <b>Time  :</b>  { data   ? timeString +' ' : null} </span>
                <span className="visit-col" span={10} > <b>Visit Duration :</b>  {data  ? data.appointment_detail.duration : "No Visit today"} </span>
            </div>
            <div className="upper-visit-row">
            ...
            <div className="video-conference-detail">
                <span className="video-col">
                   <b> Video Conference Detail</b> :<a href={"/patient"+data.video_link} target="_blank" rel="noopener noreferrer">{data.video_link}</a>
                </span>
            </div>
    ...
    catch(err)
    {
        return (
            ...
            <p className="w-100 text-start" style={{textAlign:'left',fontSize:'24px',position:'relative',left:'10px'}}><b>Visit Details</b></p>
            <div className="upper-visit-row">
                <span className="visit-col" span={10} > <b>Today's Visit  :</b> {"error in Data"} </span>
                <span className="visit-col" span={10} > <b>Visit Timing :</b>  {'error in Data'}</span>
            </div>
            <div className="upper-visit-row">
                <span className="visit-col" span={12}>
     ...
     <center>
            <Row justify="center">
            {
                    ...
                    calendarData.map((data)=>{
                        ...
                        return (<a  style={{color: "blue"}} onClick={() => onSelectedDay(data.date)}> {data.displayDay} - {data.displayMonth} - {data.displayYear} | </a>)
                      })
                      :
...
/>
            ...
            {loading && <div style={{ marginLeft: "40%", marginRight: "40%", marginTop: 50 }}><Spin tip="Loading..." size="large">
            </Spin></div>}
            ...
            {exercises.length !== 0 ? (exercises.map((ex, index) => {
                ...
                return (
                    <div key={index}
                        className="border px-2 py-6 ms-5 me-5 mt-6 exercise-card2" id="exercise-card2"
                        >
                        {ExerciseCard(ex)}
                    </div>)
            })) : !loading && <p className="text-center p border nothing-present"><b> No Plan For Today</b> </p>}
                </Col>  
