...
const PatientAI = (props) => {
  ...
  const startModel = () => {
    ...
    for (let i = 0; i < joints.length; i++) {
      if (
        selectedJoint.includes(joints[i].label) &&
        selectedJoint.includes("left") &&
        !selectedJoint.includes("Wrist")
      ) {
        ...
        selectedJoint.includes(joints[i].label) &&
        selectedJoint.includes("right") &&
        !selectedJoint.includes("Shoulder")
      ) {
        primaryAngles.push(joints[i].value);
  ...
  useEffect(() => {
    const unblock = history.block((location, action) => {
      ...
      if (
        window.confirm("Thank you for completing !!! Click on OK to continue")
      ) {
        window.darwin.stop();
        const video = document.getElementById("video");
      ...
      <>
        <Row className="px-2 py-2">
          ...
          <Col lg={8} md={8} sm={8} xs={8}>
            ...
            <p className="p">
              {currenset == exSetValue ? rep_count : currenRep} of {rep_count}
            </p>
            <p className="p">
              {currenset} of {exSetValue}
            </p>
          </Col>
        </Row>
      </>
