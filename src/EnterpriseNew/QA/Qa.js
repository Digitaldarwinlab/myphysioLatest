...
const Qa = () => {
  ...
  const [paginationState, setPaginationState] = useState({
    ...
    useEffect(() => {
        ...
        axios.post(process.env.REACT_APP_API + "/get_emp_assessment/", { id }).then(res => { console.log(res.data); setData(res.data.reverse());
            setPaginationState({
                ...paginationState,
                pageSize: 1,
 ...
 index < paginationState.maxIndex && <>
          ...
          {data.data.assesment.subjective.map((job,index) => <li>
           ...
           { data.data.posture.posture.Posterial_view && <> <div className="boxx">
