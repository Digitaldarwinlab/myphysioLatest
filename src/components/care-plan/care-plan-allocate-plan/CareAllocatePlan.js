...
import {postCarePlanAllocationEmployee} from "./../../../API/care-plan/care-plan-api";
...
const CareAllocatePlan = ({ Exercise, items, searchBar, handleChangeView }) => {
    ...
    const onFinish = async () => {
        ...
        if (state.startDate && state.endDate) {
            ...
            if (endtime.getTime() < starttime.getTime()) {
                dispatch({ type: VALIDATION, payload: { error: 'Start Date should be less than end Date' } });
                window.scrollTo({
                    top: 0,
                    behavior: 'smooth'
                });
    ...
    const onFinishEmployee = async () => {
        ...
        if (istimeValidated) {
            ...
            // }
            let result;
           const  id = episodeState.employee_code;
            result = await postCarePlanAllocationEmployee(state,dispatch,id)
    ...
    return (
        <Form form={form} layout="vertical" onFinish={episodeState.employee_code ? onFinishEmployee : onFinish} className="px-1 py-1">
            {state.isLoading && <Loading />}
            {state.success && <Success success={state.success} />}
            {validationState.error && <Error error={validationState.error} />}

            {/* <Switch defaultChecked /> */}
          {episodeState.employee_code ? <></> :  <Row >
                <Space size={"large"}>
