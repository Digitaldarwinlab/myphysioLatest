...
const PhysioRegisteration1 = (props) => {
  ...
  const handleBlur = (e) => {
    ...
    if (key === "first_name" || key == "middle_name" || key === "last_name") {
      ...
      if (error.error) {
        if (key === "first_name") error.error = "First " + error.error;
      ...
      if (error.error) {
        if (error.error) {
          if (key === "mobile_no") error.error = "Mobile " + error.error;
      ...
      if (error.error)
        dispatch({
          type: VALIDATION,
          payload: { error: "Landline " + error.error },
        });
      setTimeout(() => {
        dispatch({ type: VALIDATION, payload: { error: "" } });
      }, 10000);
  ...
  return (
    <>
      ...
      <Row>
        ...
        <Col lg={2} md={4} sm={24} xs={24}>
          <Link
            to="/physio/list"
            title="Physio List"
            className="link text-blue navlink"
            id="navlink"
          >
      ...
      <Title
        level={4}
        className="border mb-0 p-2 my-2"
          ...
          <Row gutter={[20, 20]} style={{ marginBottom: "15px" }}>
            <Col md={24} lg={12} sm={24} xs={24}>
              <FormInput
                ...
                onBlur={handleBlur}
                defaultValue={
                  state.physioRegisterReducer.landline === "NULL"
                    ? ""
