...
const Register3 = (props) => {
    ...
    const showModal = () => {
        ...
        if (checkError || allergyErr || medicalErr || familiyErr) {
            ...
            let tempData = [];
            console.log("State is ", state.BasicDetails)
            let keys = Object.keys(state.BasicDetails);
            let index = 0;
            keys.forEach(key => {
                if (!(["isLoading", "success", "pp_patm_id", "is_enterprise"].includes(key))) {
                    if (state.BasicDetails[key] !== null && state.BasicDetails[key] !== "NULL" && state.BasicDetails[key] !== "null" && (state.BasicDetails[key] !== "")) {
                        ...
                        console.log("Issue0",state.BasicDetails[key])
                        if (state.BasicDetails[key] =='pincode') {
                            console.log("Issue1",state.BasicDetails)
                            if(state.BasicDetails.pincode==0){
                                console.log("Issue2",state.BasicDetails)
                            }else{
                                tempData.push({
                                    key: index,
                                    Field: MappingKey[key],
    ...
    return (
        <>     <div style={{ minHeight: "20px" }}></div>
            ...
            <Form onFinish={onFinish} style={{ marginTop: '2%' }} layout="vertical" onFinishFailed={onFinishFailed} form={form} name="control-hooks">
                ...
                <Row justify="center">
                    <Space size={"middle"}>
                        <Col span={2}><Button className="my-3  me-3" style={{ borderRadius: '10px',backgroundColor: "#2d7ecb" }} onClick={Back}>Back</Button></Col>
                        <Col span={2}><Button htmlType="reset" className=" my-3 me-3" style={{ backgroundColor: "#2d7ecb" }} onClick={reset}>
                            {state.BasicDetails.pp_patm_id ? "Cancel" : "Reset"}
                        </Button></Col>
                        <Col span={2}><Button htmlType="submit"style={{ backgroundColor: "#2d7ecb" }} className="me-3 my-3" onClick={showModal}>
                            {state.BasicDetails.pp_patm_id ? "Update" : "Submit"}
                        </Button></Col>
                    </Space>
                    <Modal title="Confirm Details" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
