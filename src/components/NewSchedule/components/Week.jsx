...

                        {
                            Time.map((time, i) => (
                                <React.Fragment key={i}>
                                    <tr className=''>
                                        <td className='tabel_head'>{time}</td>
                                        {
                                            weeks.map((week, i) => (
                                                <td key={i} onClick={() => {
                                                    const newDate = currentWeek.toDate().toUTCString()
                                                    if(time[6] === 'P' && time.slice(0,2)!=='12'){
                                                        console.log('From P',Arr[i]+ newDate.slice(7,17) + (+time.slice(0,2)+ 6)+':30:00' + newDate.slice(25,29))
                                                        dispatch({type:'VISIT_DATE',payload:{date:moment((+Arr[i])+newDate.slice(7,17) + (+time.slice(0,2)+ 6)+':30:00' + newDate.slice(25,29))}})
                                                      } else {
                                                        console.log('From A',newDate.slice(7,17) + (+time.slice(0,2) - 6)+':30:00' + newDate.slice(25,29))
                                                        dispatch({type:'VISIT_DATE',payload:{date:moment((+Arr[i])+newDate.slice(7,17) + (+time.slice(0,2) - 6)+':30:00' + newDate.slice(25,29))}})
                                                      }
                                                    setIsVisible(true)
                                                }} className='week_line' id={time}
                                                >{data.filter(d => new Date(d.startDate).getDate() === +Arr[i] && new Date(d.startDate).getMonth() === currentWeek.toDate().getMonth()  && tConvert(d.startTime)[8] === time[6] && tConvert(d.startTime).includes(time.slice(0,3)) && d.startTime.slice(3,5) < 15 ).map((d,i) =>  <button key={d.id} onClick={() =>handleVisitClick(d)}>{d.patient || 'Is Present'}</button>)}</td>
                                            ))
                                        } 
                                    </tr>
                                    <tr className=''>
                                            ...
                                            weeks.map((week, i) => (
                                                <td key={i} id={time} onClick={() => {
                                                    ...
                                                    if(time[6] === 'P' && time.slice(0,2)!=='12'){
                                                      ...
                                                      dispatch({type:'VISIT_DATE',payload:{date:moment((+Arr[i])+newDate.slice(7,17) + (+time.slice(0,2)+ 6)+':45:00'  + newDate.slice(25,29))}})
                                                    } else {
                                                    ...
                                                    className='week_line'>{data.filter(d => new Date(d.startDate).getDate() === +Arr[i] && new Date(d.startDate).getMonth() === currentWeek.toDate().getMonth()  && tConvert(d.startTime)[8] === time[6] && tConvert(d.startTime).includes(time.slice(0,3)) && d.startTime.slice(3,5) >= 15 && d.startTime.slice(3,5) < 30 ).map((d,i) =>  <button key={d.id} onClick={(() => {handleVisitClick(d)})}>{d.patient || 'Is Present'} </button>)}</td>
                                            ))
                                        } 
                                    </tr>
                                    <tr className=''>
                                        <td  className='tabel_head' ></td>
                                        {
                                            weeks.map((week, i) => (
                                                <td key={i} id={time} onClick={() => {
                                                    ...
                                                    className='week_line'>{data.filter(d => new Date(d.startDate).getDate() === +Arr[i]  && new Date(d.startDate).getMonth() === currentWeek.toDate().getMonth()  && tConvert(d.startTime)[8] === time[6] && tConvert(d.startTime).includes(time.slice(0,3)) && d.startTime.slice(3,5) >= 30 && d.startTime.slice(3,5) < 45 ).map((d,i) =>  <button key={d.id} onClick={(() => {handleVisitClick(d)})}>{d.patient || 'Is Present'} </button>)}</td>
                                            ))
                                        } 
                                    </tr>
                                    <tr className=''>
                                        <td  className='tabel_head' ></td>
                                        {
                                            weeks.map((week, i) => (
                                                <td key={i} id={time} onClick={() => {
                                                    ...
                                                    if(time[6] === 'P' && time.slice(0,2)!=='12'){
                                                      console.log('From P',newDate.slice(0,17) + (+time.slice(0,2)+ 6)+':30:00' + newDate.slice(25,29))
                                                      dispatch({type:'VISIT_DATE',payload:{date:moment((+Arr[i])+newDate.slice(7,17) + (+time.slice(0,2)+ 7)+':15:00' + newDate.slice(25,29))}})
                                                    } else {
                                                      console.log('From A',newDate.slice(0,17) + (+time.slice(0,2) - 6)+':30:00' + newDate.slice(25,29))
                                                      dispatch({type:'VISIT_DATE',payload:{date:moment((+Arr[i])+newDate.slice(7,17) + (+time.slice(0,2) - 5)+':15:00' + newDate.slice(25,29))}})
                                                    }
                                                    setIsVisible(true)
                                                }}
                                                    className='week_line'>{data.filter(d => new Date(d.startDate).getDate() === +Arr[i]  &&  new Date(d.startDate).getMonth() === currentWeek.toDate().getMonth()  && tConvert(d.startTime)[8] === time[6] && tConvert(d.startTime).includes(time.slice(0,3)) && d.startTime.slice(3,5) >= 45 && d.startTime.slice(3,5) < 60 ).map((d,i) =>  <button key={d.id} onClick={(() => {handleVisitClick(d)})}>{d.patient || 'Is Present'} </button>)}</td>
                                            ))
                                        } 
                                    </tr>
                                    {/* <tr >
