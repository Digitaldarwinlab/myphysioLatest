...
const handleVisitClick = (data) => {
  ...
  return (
                ...
                <td
                  ...
                  onClick={(e) => {
                    ...
                    if(t[6] === 'P' && t.slice(0,2)!=='12'){
                      console.log('From P',todayDate+newDate.slice(0,17) + (+t.slice(0,2)+ 7)+':30:00' + newDate.slice(25,29))
                      dispatch({type:'VISIT_DATE',payload:{date:moment(todayDate+newDate.slice(7,17) + (+t.slice(0,2)+ 6)+':30:00' + newDate.slice(25,29))}})
                    } else {
                      console.log('From A',todayDate+newDate.slice(7,17) + (+t.slice(0,2) - 5)+':30:00' + newDate.slice(25,29))
                      dispatch({type:'VISIT_DATE',payload:{date:moment(todayDate+newDate.slice(7,17) + (+t.slice(0,2) - 6)+':30:00' + newDate.slice(25,29))}})
                    }
    ...
    // console.log(new Date().toUTCString().slice(25,29))
                ...
                >
                  {data.filter(d => new Date(d.startDate).getDate() === todayDate &&tConvert(d.startTime)[8] === t[6] && tConvert(d.startTime).includes(t.slice(0,3)) && d.startTime.slice(3,5) < 15).map((d,i) =>  <button  onClick={() => handleVisitClick(d)}>{d.patient}</button>)}
              ...
              <tr>
                ...
                <td
                  ...
                  onClick={(e) => {
                    ...
                    const newDate = new Date().toUTCString()
                    if(t[6] === 'P' && t.slice(0,2)!=='12'){
                      console.log('From P',newDate.slice(0,17) + (+t.slice(0,2)+ 6)+':30:00' + newDate.slice(25,29))
                      dispatch({type:'VISIT_DATE',payload:{date:moment(todayDate+newDate.slice(4,17) + (+t.slice(0,2)+ 6)+':45:00' + newDate.slice(25,29))}})
                    } else {
                      console.log('From A',newDate.slice(0,17) + (+t.slice(0,2) - 6)+':30:00' + newDate.slice(25,29))
                      dispatch({type:'VISIT_DATE',payload:{date:moment(todayDate+newDate.slice(4,17) + (+t.slice(0,2) - 6)+':45:00' + newDate.slice(25,29))}})
                    }
                ...
                > {data.filter(d => new Date(d.startDate).getDate() === todayDate &&tConvert(d.startTime)[8] === t[6] && tConvert(d.startTime).includes(t.slice(0,3)) && d.startTime.slice(3,5) >= 15 && d.startTime.slice(3,5) < 30 ).map((d,i) =>
                <button key={d.id} onClick={() => handleVisitClick(d)}>{d.patient}</button>)}</td>
              </tr>
              <tr>
                <td className="col"></td>
                <td
                  ...
                  onClick={(e) => {
                    ...
                    if(t[6] === 'P' && t.slice(0,2)!=='12'){
                      console.log('From P',newDate.slice(0,17) + (+t.slice(0,2)+ 6)+':30:00' + newDate.slice(25,29))
                      dispatch({type:'VISIT_DATE',payload:{date:moment(todayDate+newDate.slice(4,17) + (+t.slice(0,2)+ 7)+':00:00' + newDate.slice(25,29))}})
                    } else {
                      console.log('From A',newDate.slice(0,17) + (+t.slice(0,2) - 6)+':30:00' + newDate.slice(25,29))
                      dispatch({type:'VISIT_DATE',payload:{date:moment(todayDate+newDate.slice(4,17) + (+t.slice(0,2) - 5)+':00:00' + newDate.slice(25,29))}})
                    }
                ...
                > {data.filter(d => new Date(d.startDate).getDate() === todayDate &&tConvert(d.startTime)[8] === t[6] && tConvert(d.startTime).includes(t.slice(0,3)) && d.startTime.slice(3,5) >= 30 && d.startTime.slice(3,5) < 45 ).map((d,i) =>
                <button key={d.id} onClick={() => handleVisitClick(d)}>{d.patient}</button>)}</td>
              </tr>
              <tr>
                <td className="col"></td>
                <td
                  ...
                  onClick={(e) => {
                    ...
                    if(t[6] === 'P' && t.slice(0,2)!=='12'){
                      console.log('From P',newDate.slice(0,17) + (+t.slice(0,2)+ 6)+':30:00' + newDate.slice(25,29))
                      dispatch({type:'VISIT_DATE',payload:{date:moment(todayDate+newDate.slice(4,17) + (+t.slice(0,2)+ 7)+':15:00' + newDate.slice(25,29))}})
                    } else {
                      console.log('From A',newDate.slice(0,17) + (+t.slice(0,2) - 6)+':30:00' + newDate.slice(25,29))
                      dispatch({type:'VISIT_DATE',payload:{date:moment(todayDate+newDate.slice(4,17) + (+t.slice(0,2) - 5)+':15:00' + newDate.slice(25,29))}})
                    }
                ...
                > {data.filter(d => new Date(d.startDate).getDate() === todayDate &&tConvert(d.startTime)[8] === t[6] && tConvert(d.startTime).includes(t.slice(0,3)) && d.startTime.slice(3,5) >= 45 && d.startTime.slice(3,5) < 60 ).map((d,i) =>
                <button key={d.id} onClick={() => handleVisitClick(d)}>{d.patient}</button>)}</td>
              </tr>
            </>
          ))}
