...
const EpisodeVisitDetails = () => {
  ...
  const [activetabindex, Setactivetabindex] = useState(
    history.location.state ? history.location.state.tab : "1 "
  );
  ...
  useEffect(() => {
    ...
    async function getPatients() {
      ...
      if (state.patient_code) {
        ...
        console.log(state.patient_code);
      }
    }
    getPatients();
    ...
    if (history.location.state) {
      if (history.location.state.prevPath === "/visit")
        if (!isNaN(history.location.state.id)) {
          const res = await Patient_profile(
    ...
    return (
      <Row className="pat_details_mobile">
        ...
        <Col lg={6} md={6} sm={12} xs={24}>
          <p className="fw-bold">
            {" "}
            <strong>Patient Name :</strong> {state.patient_name}
          </p>
          {/* <div className="border rounded " style={{maxHeight:'45px'}}>
        ...
        {userInfo.role != "physio" && (
          <Col lg={6} md={6} sm={12} xs={24}>
            <p className="fw-bold">
              <strong> Contact Number : </strong> {state.Patient_no}{" "}
            </p>
          </Col>
        )}
      </Row>
  ...
  const DetailTabs = () => {
    return (
      <div className="card card-container PatientDetails">
        <Tabs
          defaultActiveKey={activetabindex == 2 ? "2" : "1"}
          type="card"
          size="medium"
        >
          <TabPane
          ...
          <TabPane
            tab={<div className="fw-bold ant-tabs-btn">Episodes</div>}
            key="2"
          >
            <Episodes handleClick2={episodeClick} />
          </TabPane>
          <TabPane
            tab={<div className="fw-bold ant-tabs-btn">Visit</div>}
            key="3"
          >
            <Visits handleClick={handleClick} patId={patId} change={change} />
          </TabPane>
          <TabPane
            tab={<div className="fw-bold ant-tabs-btn">Assessment</div>}
            key="4"
          >
            {/* aswin start 10/30/2021 start */}
            <AssessmentList
              assesmentClick={assesmentClick}
              patientId={state.patient_code}
            />
            {/* aswin start 10/30/2021 stop */}
          </TabPane>
          <TabPane
            tab={<div className="fw-bold ant-tabs-btn">Care Plan</div>}
            key="6"
          >
            {/* aswin start 10/30/2021 start */}
            <CarePlanView
              carePlanClick={carePlanClick}
              eid={carePlanState.pp_ed_id}
              searchBar={false}
            />
            {/* aswin start 10/30/2021 stop */}
          </TabPane>
          ...
          <TabPane
            tab={<div className="fw-bold ant-tabs-btn">Dashboard</div>}
            key="8"
          >
          ...
          >
            {/* aswin start 10/30/2021 start */}
            <Prescriptions
              prescriptionClick={prescriptionClick}
              dashboard={true}
              eid={carePlanState.pp_ed_id}
            />
            {/* aswin start 10/30/2021 stop */}
          ...
          <TabPane
            tab={<div className="fw-bold ant-tabs-btn">Notes</div>}
            key="7"
          >
  ...
  return (
    <div
      className=""
      style={{ maxWidth: "100%", msOverflowX: "hidden" }}
      ref={episodeRef}
    >
      {Header()}
      <div className="rest">
