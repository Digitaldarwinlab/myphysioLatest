    ...
    return (
            ...
            <Row className="m-2">
                ...
                <Col lg={6} md={6} sm={6} xs={24} className="text-end">
                    ...
                    <Button className="button1" style={{color: "white"}} id="bnid"
                        onClick={prescriptionClick}><ImPlus className="me-2"/> {"  "}Add</Button>
                </Col>
            </Row>
       ...
       } */}
       {
            ...
            <Col span={24} className="px-3">
                {loading && <Loading />}
                                {/* {prescriptions === null ||prescriptions === undefined||prescriptions.length<0 && <p className="fw-bold">No Assesment Present..</p>} */}
                                {prescriptions.length === 0 ? <p className="fw-bold">No Assesment Present..</p> : (
                    <>
                        <div  className="border p-1 m-2">
                ...
                <Col >
                    <p className="fw-bold" style={{fontSize:'18px'}}><b> Note : </b>{data.notes}</p>
                </Col>
...

            {
                    ...
                    <h4> <b> Instructions  </b> :   {data.medication_detail ? data.medication_detail.length>0 ?  data.medication_detail[0].instruction : 'No MEdication' : 'No MEdication'}{data.medication_detail[0].instruction.length===0&&"No Instruction Available"}</h4>
                    <h4> <b> Medicine  </b> :    {data.medication_detail ? data.medication_detail.length>0 ? data.medication_detail.map(item=>{return item.medic_notes})  : null : null} </h4>
                    <h4> <b> Medication Note  </b> :    {data.medication_detail ? data.medication_detail.length>0 ? data.medication_detail.map(item=>{return item.medic_notes}) : null : null} </h4>
                    </p>  
                    </Col> 
