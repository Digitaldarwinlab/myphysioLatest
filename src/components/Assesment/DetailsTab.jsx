  ...
  return (
         <Col span={24}>
                    <div className='patient-details-1x h3-font-1x bg-theme-1x div-border-1x'>
                        <span>Patient Name : {state.episodeReducer.patient_name?state.episodeReducer.patient_name:'no data'}</span>
                        <span>Patient Episode :  {episodedata ? episodedata.complaintId : null}</span>
                        <span>Patient Code : {state.episodeReducer.patient_main_code?state.episodeReducer.patient_main_code:'no data'}</span>
                    </div>
                </Col>
  )
}
