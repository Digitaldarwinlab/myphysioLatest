...
const Questions = () => {
    ...
    const handleChange = (key, value, index = 0, ques, ans) => {
        if (key === "Symptoms" || key === "pain" || key === "DailyLiving" || key === "Sports" || key === "Life" || key === "Stiffness") {
            dispatch({
    ...
    const HandleSubmit = () => {
        ...
        if (SymptomsArray.length != symptomsQuestion.questions.length ||
            StifnessArray.length != stiffnessQuestion.questions.length ||
            PainArray.length != painQuestion.questions.length ||
            ADLArray.length != dailyLivingQuestion.questions.length ||
            SportsArray.length != sportsQuestion.questions.length ||
            QOLArray.length != lifeQuestion.questions.length) {
            notification.error({
        ...
        <>
            ...
            <Form>
                <Collapse defaultActiveKey={['1']} >
                    <Panel header="Symptons" key="1" className="bold">
                        <div className="ps-4">
                            <h5>{symptomsQuestion.description}</h5>

                            <Row>
                                <Col md={18} lg={18} sm={24} xs={24}>

                                    {symptomsQuestion.questions.map((ques, key) => {
                    ...
                    <Panel header="Stiffness" key="2">

                        <div className="ps-4">
                            <h5>{stiffnessQuestion.description}</h5>

                            <Row>
                                <Col md={18} lg={18} sm={24} xs={24}>
                                    {stiffnessQuestion.questions.map((ques, key) => {
                                        return (
                                            <>
                                                <p className="mt-2">
                                                    {ques[0]}
                    ...
                    <Panel header="Pain" key="3">

                        <div className="ps-4">
                            <h5>{painQuestion.description}</h5>

                            <Row>
                                <Col md={18} lg={18} sm={24} xs={24}>
                                    {painQuestion.questions.map((ques, key) => {
                                        return (
                                            <>
                                                <p className="mt-2">
                                                    {ques[0]}
                    ...
                    <Panel header="Function, daily living" key="4">
                        <div className="ps-4">
                            <h5>{dailyLivingQuestion.description}</h5>

                            <Row>
                                <Col md={18} lg={18} sm={24} xs={24}>
                                    {dailyLivingQuestion.questions.map((ques, key) => {
                                        return (
                                            <>
                                                <p className="mt-2">
                                                    {ques[0]}
                    ...
                    <Panel header="Function, sports and recreational activities " key="5">
                        <div className="ps-4">
                            <h5>{sportsQuestion.description}</h5>

                            <Row>
                                <Col md={18} lg={18} sm={24} xs={24}>
                                    {sportsQuestion.questions.map((ques, key) => {
                                        return (
                                            <>
                                                <p className="mt-2">
                                                    {ques[0]}
                    ...
                    <Panel header="Quality of Life " key="6">
                        <div className="ps-4">
                            <h5>{lifeQuestion.description}</h5>

                            <Row>
                                <Col md={18} lg={18} sm={24} xs={24}>
                                    {lifeQuestion.questions.map((ques, key) => {
                                        return (
                                            <>
                                                <p className="mt-2">
                                                    {ques[0]}
